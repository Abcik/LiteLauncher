...
public final class MicrosoftCallbackServer {
    ...
    public void close() {
        ...
        if (closingPending != null) {
            ...
            closingPending.page().complete(MicrosoftCallbackPage.errorHtml(closingPending.theme(), closingPending.language(), InformationMessages.WEB_AUTH_ERROR));
        }
    ...
    private void handleCallback(HttpExchange exchange) throws IOException {
        ...
        if (auth == null) {
            writePage(exchange, 400, MicrosoftCallbackPage.errorHtml(Theme.LIGHT, Language.ENGLISH, InformationMessages.WEB_AUTH_ERROR));
            return;
        }
        ...
        try {
            ...
            writePage(exchange, 500, MicrosoftCallbackPage.errorHtml(auth.theme(), auth.language(), InformationMessages.WEB_AUTH_ERROR));
        }
    }
    ...
    private void completeWithError(HttpExchange exchange, PendingMicrosoftAuth auth, String message) throws IOException {
        ...
        writePage(exchange, 400, MicrosoftCallbackPage.errorHtml(auth.theme(), auth.language(), InformationMessages.WEB_AUTH_ERROR));
    }
